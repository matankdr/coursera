...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = StructType {
    StructField(name = columnNames.head, dataType = StringType, nullable = false) ::
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val groups = columnNames.foldLeft(List.empty[(Int, Column)]){(acc, name) =>
        categories.zipWithIndex.flatMap {
          case (prefixes, index) if prefixes.exists(name.startsWith) => Some((index, new Column(name)))
          case _ => None
        }
        .sortBy{ case (index, _) => index }
        .headOption match {
          case Some(row) => row :: acc
          case None => acc
        }
      }
      .groupBy{case (index, row) => index}
      .mapValues(_.map(_._2))


    val results = (0 to categories.size).map(index => groups.getOrElse(index, List.empty[Column]))
