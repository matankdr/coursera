...
object Server {
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] = {
    implicit val eventOrdering = new Ordering[Event] {
      def compare(e1: Event, e2: Event): Int =
        e1.sequenceNr compare e2.sequenceNr
    }

    @tailrec
    def takeBatch(buffer: SortedSet[Event], nextSeq: Int, agg: List[Event]): List[Event] = {
      if (buffer.isEmpty) agg
      else if (nextSeq == buffer.head.sequenceNr) takeBatch(buffer.tail, nextSeq + 1, agg :+ buffer.head)
      else agg
    }

    Flow[Event].statefulMapConcat[Event] { () =>
      ...
      event => {
        ...
        val result = takeBatch(buffer, nextSeq, List.empty[Event])
        buffer = buffer.drop(result.size)
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] =
    Flow[Event].statefulMapConcat { () =>
      ...
      event => {
        event match {
          case Follow(_, fromId, toId) =>
            val followed = followers(fromId) + toId
            followers += (fromId -> followed)

          case Unfollow(_, fromId, toId) =>
            val followed = followers(fromId) - toId
            followers += fromId -> followed
        ...
        scala.collection.immutable.Iterable((event, followers))
      }
    }
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean =
    eventAndFollowers match {
      case (Follow(_, _, to), _) if userId == to => true
      case (Broadcast(_), _) => true
      case (PrivateMsg(_, _, to), _) if userId == to => true
      case (StatusUpdate(_, from), followers) =>
        followers.exists{ case (_, f) => f contains from }
      case _ => false
    }
...
class Server()(implicit executionContext: ExecutionContext, materializer: Materializer)
  ...
  def outgoingFlow(userId: Int): Source[ByteString, NotUsed] =
    broadcastOut.filter {
      case (Follow(_, _, toUserId), _) if toUserId == userId => true
      case (Unfollow(_, _, _), _) => false
      case (Broadcast(_), _) => true
      case (PrivateMsg(_, _, to), _) if userId == to => true
      case (StatusUpdate(_, from), followers)  =>
        followers(userId) contains from
      case _ => false
...
//    clientIdPromise.future.map(id => actorSystem.log.info("Connected follower: {}", id.userId))
    ...
    val incoming: Sink[ByteString, NotUsed] =
      identityParserSink.mapMaterializedValue { identity =>
        clientIdPromise.completeWith(identity)
        NotUsed
      }
