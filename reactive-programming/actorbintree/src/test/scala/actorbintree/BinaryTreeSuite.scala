...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuiteLike with Matchers with BeforeAndAfterAll with ImplicitSender
{
  ...
  test("my example") {
    ...
    val requesterRef = requester.ref
    val ops = List(
      Contains(requesterRef, id=50, 100),
      Insert(requesterRef, id=100, 100),
      Insert(requesterRef, id= 200, 200),
      Insert(requesterRef, id= -20, -20),
      Insert(requesterRef, id= 10, 100),
      Insert(requesterRef, id= 11, 0),
      Contains(requesterRef, id=80, -20),
      Contains(requesterRef, id=81, 0),
      Contains(requesterRef, id=70, 200)
    )
    ...
    val expectedReplies = List(
      ...
      ContainsResult(id=50, false),
      ContainsResult(id=70, true),
      ContainsResult(id=80, true),
      ContainsResult(id=81, true),
      OperationFinished(id=100),
      OperationFinished(id=200)
    )

    verify(requester, ops, expectedReplies)
  }

  test("my example with GC") {
    val requester = TestProbe()
    val requesterRef = requester.ref
    val topNode = system.actorOf(Props[BinaryTreeSet])

    topNode ! Insert(requesterRef, id=100, 100)
    topNode ! Insert(requesterRef, id=200, 200)
    topNode ! GC
    Contains(requesterRef, id=201, 200)
    topNode ! Insert(requesterRef, id=0, 0)
    //    topNode ! Contains(requesterRef, id=2, 0)
