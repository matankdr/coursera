...
object ParallelCountChange {
  ...
  def countChange(money: Int, coins: List[Int]): Int = {
    if (money < 0) 0
      ...
      case Nil => 0
      case currCoin :: withoutCurrCoin =>
        countChange(money - currCoin, coins) + countChange(money, withoutCurrCoin)
    }
  }
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (threshold(money, coins)) {
      ...
      if (money < 0) 0
        ...
        case Nil => 0
        case currCoin :: withoutCurrCoin =>
          val (resLeft, resRight) = parallel(parCountChange(money - currCoin, coins, threshold),
            parCountChange(money, withoutCurrCoin, threshold))
