...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    input.zipWithIndex.foreach {
      case (currPoint, 0) => output(0) = 0
      case (currPoint, i) => output(i) = max( currPoint / i, output(i - 1) )
    }
  ...
  def upsweepSequential(input: Array[Float], from: Int, to: Int): Float = {
    (from until to).foldLeft(0f)((currMax, i) => max(input(i) / i, currMax) )
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if (end - from <= threshold) {
      ...
      val pivot = from + (end - from) / 2
      val (left, right) = parallel(
        upsweep(input, from, pivot, threshold),
        upsweep(input, pivot, end, threshold))
      Node(left, right)
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    startingAngle: Float, from: Int, to: Int): Unit = {

    @tailrec
    def iterate(i: Int, end: Int, currMax: Float): Unit = {
      if (i < end) {
        output(i) = max(input(i) / i, currMax)
        iterate(i + 1, end, output(i))
      }
    }

    iterate(from, to, startingAngle)
  }
