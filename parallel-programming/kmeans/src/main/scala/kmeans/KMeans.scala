...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val pointsGroupedByMeans = points.par.groupBy(findClosest(_, means))

    means.par.map(mean =>
      mean -> pointsGroupedByMeans.getOrElse( mean, GenSeq() )
    ).toMap
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans).forall { case (oldM, newM) =>
      (oldM squareDistance newM) <=  eta
    }
  }
